//
//  BackupManagerViewController.swift
//  PS Mac Tools
//
...
class BackupManager3ViewController: NSViewController, NSTableViewDelegate, NSTableViewDataSource {
    ...
    @IBOutlet weak var titleIDTextField: NSTextField!
    @IBOutlet weak var requiredFWTextField: NSTextField!
    @IBOutlet weak var appVersionTextField: NSTextField!
    @IBOutlet weak var gameFormatTextField: NSTextField!
    @IBOutlet weak var gamesTableView: NSTableView!
    @IBOutlet weak var backgroundImage: NSImageView!
    
    var gameTitle: String = ""
    var gameTitleID: String = ""
    var gameRequiredFW: String = ""
    var gameVer: String = ""
    var gameCatergory: String = ""
    var gameResolutions: String = ""
    var gameSoundFormats: String = ""
    
    var currentGamesPath: String = ""
    var gamesList = [PS3Game]()
    ...
    @IBAction func loadBackups(_ sender: NSButton) {
            ...
            for foundGame in searchGameSFOorISO(pathURL: result!)
            {
                if URL(string: foundGame)?.pathExtension == "iso" {
                    ...
                    if directoryExistsAtPath(currentGamesPath + "/PSMT/" + gameFolderName) {
                        ...
                        gamesList.append(parseSFO(inputFile: currentGamesPath + "/PSMT/" + gameFolderName + "/PS3_GAME/PARAM.SFO", fullGamePath: URL(string: gamePath)!.path))
                    }
                    else
                    {
    ...
    func parseSFO(inputFile: String, fullGamePath: String) -> PS3Game {
        ...
        var SFOgameReqFW = SFOReaderOutput.filter({(item: String) -> Bool in
            let stringMatch = item.lowercased().range(of: "PS3_SYSTEM_VER=".lowercased())
             return stringMatch != nil ? true : false
        })
        ...
        let newGame = PS3Game(title: SFOgameTitle[0].components(separatedBy: "=")[1],
                              ID: SFOgameID[0].components(separatedBy: "=")[1],
                              ver: SFOgameVersion[0].components(separatedBy: "=")[1],
                              reqFW: SFOgameReqFW[0].components(separatedBy: "=")[1],
                              type: SFOgameType[0].components(separatedBy: "=")[1],
                              parentalLevel: SFOgameParentalLevel[0].components(separatedBy: "=")[1],
                              completeGamePath: fullGamePath)
    ...
    func extractFilesfromISO(filePath: String, backupDirectory: String) {
        ...
        let currentGameSFO = backupDirectory + "/PSMT/" + GameFolderName + "/PS3_GAME/PARAM.SFO"
    ...
    func tableView(_ tableView: NSTableView, viewFor tableColumn: NSTableColumn?, row: Int) -> NSView? {
            ...
            if tableColumn?.identifier == NSUserInterfaceItemIdentifier(rawValue: "titleColumn") {
            ...
            else if tableColumn?.identifier == NSUserInterfaceItemIdentifier(rawValue: "titleIDColumn") {
         
                let cellIdentifier = NSUserInterfaceItemIdentifier(rawValue: "gameTitleIDTextField")
                guard let cellView = tableView.makeView(withIdentifier: cellIdentifier, owner: self) as? NSTableCellView else { return nil }
                cellView.textField?.stringValue = gamesList[row].ID
                return cellView
            }
                ...
                guard let cellView = tableView.makeView(withIdentifier: cellIdentifier, owner: self) as? NSTableCellView else { return nil }
                cellView.textField?.stringValue = gamesList[row].completeGamePath
                return cellView
            }
            else
            {
    ...
    func tableViewSelectionDidChange(_ notification: Notification) {
        ...
        selectedGamePath = gamesList[gamesTableView.selectedRow].completeGamePath
        
        let correctGameID = gamesList[gamesTableView.selectedRow].ID.components(separatedBy: CharacterSet.letters.inverted).joined() + "-" + gamesList[gamesTableView.selectedRow].ID.components(separatedBy: CharacterSet.decimalDigits.inverted).joined()
        
        titleIDTextField.stringValue = "ID: " + correctGameID
        
        requiredFWTextField.stringValue = "Required FW: " +  gamesList[gamesTableView.selectedRow].reqFW.replacingOccurrences(of: "0", with: "")
        appVersionTextField.stringValue = "Game Version: " + gamesList[gamesTableView.selectedRow].ver
        gameFormatTextField.stringValue = "Game Type: " + gamesList[gamesTableView.selectedRow].type
        
        if !gamesList[gamesTableView.selectedRow].completeGamePath.contains(".iso") {
        
            if fileExistsAtPath(gamesList[gamesTableView.selectedRow].completeGamePath + "/PS3_GAME/ICON0.PNG") {
                    gameImage.image = NSImage(byReferencingFile: gamesList[gamesTableView.selectedRow].completeGamePath + "/PS3_GAME/ICON0.PNG")
                }
                else {
                    gameImage.image = nil
            }
                    
                if fileExistsAtPath(gamesList[gamesTableView.selectedRow].completeGamePath + "/PS3_GAME/PIC1.PNG") {
                    backgroundImage.image = NSImage(byReferencingFile: gamesList[gamesTableView.selectedRow].completeGamePath + "/PS3_GAME/PIC1.PNG")
                }
                else {
                    backgroundImage.image = nil
            }
            
            if fileExistsAtPath(gamesList[gamesTableView.selectedRow].completeGamePath + "/PS3_GAME/SND0.AT3") {
                    let ffPlay = Bundle.main.path(forResource: "ffplay", ofType: "")
                    backgroundAudioPlayer.launchPath = "/bin/sh"
                    backgroundAudioPlayer.arguments = ["-c", "'" + ffPlay! + "' -nodisp -autoexit '" + gamesList[gamesTableView.selectedRow].completeGamePath + "/PS3_GAME/SND0.AT3'"]
                    backgroundAudioPlayer.launch()
            }
            else if fileExistsAtPath(gamesList[gamesTableView.selectedRow].completeGamePath + "/SND0.AT3") {
                    let ffPlay = Bundle.main.path(forResource: "ffplay", ofType: "")
                    backgroundAudioPlayer.launchPath = "/bin/sh"
                    backgroundAudioPlayer.arguments = ["-c", "'" + ffPlay! + "' -nodisp -autoexit '" + gamesList[gamesTableView.selectedRow].completeGamePath + "/SND0.AT3'"]
                    backgroundAudioPlayer.launch()
            }
            ...
            let selectedGameISO = URL(fileURLWithPath: gamesList[gamesTableView.selectedRow].completeGamePath)
            ...
            if fileExistsAtPath(gameCacheFolder + selectedGameName + "/PS3_GAME/ICON0.PNG") {
                    gameImage.image = NSImage(byReferencingFile: gameCacheFolder + selectedGameName + "/PS3_GAME/ICON0.PNG")
                }
                else {
                    gameImage.image = nil
            }
                    
                if fileExistsAtPath(gameCacheFolder + selectedGameName + "/PS3_GAME/PIC1.PNG") {
                    backgroundImage.image = NSImage(byReferencingFile: gameCacheFolder + selectedGameName + "/PS3_GAME/PIC1.PNG")
                }
                else {
                    backgroundImage.image = nil
            }
            
            if fileExistsAtPath(gameCacheFolder + selectedGameName + "/PS3_GAME/SND0.AT3") {
                let ffPlay = Bundle.main.path(forResource: "ffplay", ofType: "")
                backgroundAudioPlayer.launchPath = "/bin/sh"
                backgroundAudioPlayer.arguments = ["-c", "'" + ffPlay! + "' -nodisp -autoexit '" + gameCacheFolder + selectedGameName + "/PS3_GAME/SND0.AT3'"]
                backgroundAudioPlayer.launch()
            }
    ...
    override func prepare(for segue: NSStoryboardSegue, sender: Any?) {
        ...
        if segue.identifier == "copySegue" {
            
            let newCopyWindow = segue.destinationController as! CopyWindowViewController
            newCopyWindow.sourceFolder = selectedGamePath
            
            if backgroundAudioPlayer.isRunning {
                newCopyWindow.isBackupManagerMusicPlaying = true
                newCopyWindow.backgroundAudioPlayerCopy = backgroundAudioPlayer
            }
            else
            {
                newCopyWindow.isBackupManagerMusicPlaying = false
            }
